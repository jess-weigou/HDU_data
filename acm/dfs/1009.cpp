...
int main()
{
	...
	while(scanf("%d",&n),n)
	{
		...
		{
			scanf("%d%d%d%d",&mp[i].from,&mp[i].to,&mp[i].value,&mp[i].flag);
			if(mp[i].flag==1) merge(father,mp[i].from,mp[i].to);
		}
		qsort(mp,n*(n-1)/2,sizeof(mp[0]),cmp);//从第一个开始排序 
		int sum=0;
		for(int i=0;i<n*(n-1)/2;i++)
		{
			if(find(father,mp[i].from)!=find(father,mp[i].to)&&mp[i].flag==0)
			{
